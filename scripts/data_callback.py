...
def data_callback(count_r, angle_r, delta_r):
	if angle_r not in variables.totals:		
		variables.totals[angle_r] = variables.CBData(count_r, delta_r)
	else:
		oldvals_c = variables.totals[angle_r].count
		oldvals_d = variables.totals[angle_r].delta
		variables.totals[angle_r] =  variables.CBData(count_r + oldvals_c, delta_r + oldvals_d)
